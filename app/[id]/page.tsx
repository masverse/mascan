...
export default async function Page({ params }: { params: { id: string } }) {
    async function search() {
        const res = await fetch(`${process.env.NEXT_FETCH_URL}${params.id}`)
        if (!res.ok) {
        ...
        <>
            <div className="flex flex-col items-center w-full">
                <div className="w-full px-10 md:px-24 lg:px-56">
                    <p className="text-xl md:text-3xl font-bold mt-8 md:mt-16">Transaction Details</p>
                    ...
                    <div className="text-[13px] md:text-[16px] rounded-3xl flex flex-col items-center mt-8 h-fit border-0 border-gray-200 break-all text-white bg-white/30 backdrop-blur-md">
                        <div className="flex font-semibold w-full px-5 py-4 md:px-16 justify-between items-center gap-6">
                            <p className="flex justify-start break-normal">You're able to request for access. Click here to request for access</p>
                            <button className="bg-gray-800 outline-none text-white rounded-lg md:px-5 md:py-2 px-2 py-2 w-fit break-keep">Request</button>
                        </div>
